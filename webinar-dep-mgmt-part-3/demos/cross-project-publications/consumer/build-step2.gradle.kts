...
tasks.register("resolveInstrumentedClasses") {
    ...
    doLast {
        configurations.testCompileClasspath.describe()
        configurations.testRuntimeClasspath.describe()
    }
}

fun NamedDomainObjectProvider<Configuration>.describe() = get().run {
    val projectPath = rootProject.projectDir.absolutePath
    println(files.map {
        if (it.isFile) {
            it.name
        } else {
            it.absolutePath.replace(projectPath, "")
        }
    })
}
